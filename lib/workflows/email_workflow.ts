...
export async function GenerateEmailResponse(
  ...
  try {
    ...
    let responseSubject = subject.startsWith("Re:")
      ? subject
      : `Re: ${subject}`;
    ...
    console.error(
      "[GenerateEmailResponse] Error generating email response:",
      error
    );
    throw error;
  }
 ...
 */
export async function SendEmailFromAgent(emailDetails: {
  subject: string;
  body: string;
  recipient_address: string;
}): Promise<string> {
  console.log(
    `[SendEmailFromAgent] Attempting to send email to: ${emailDetails.recipient_address}`
  );
  console.log(`[SendEmailFromAgent] Email subject: ${emailDetails.subject}`);
  console.log(
    `[SendEmailFromAgent] Email body preview: ${emailDetails.body.substring(
      0,
      100
    )}...`
  );
  ...
  if (!A1BASE_ACCOUNT_ID || !A1BASE_AGENT_EMAIL) {
    console.error(
      "[SendEmailFromAgent] Missing A1Base Account ID or Agent Email in environment variables."
    );
    console.error(
      `[SendEmailFromAgent] A1BASE_ACCOUNT_ID: ${
        A1BASE_ACCOUNT_ID ? "SET" : "MISSING"
      }`
    );
    console.error(
      `[SendEmailFromAgent] A1BASE_AGENT_EMAIL: ${
        A1BASE_AGENT_EMAIL ? A1BASE_AGENT_EMAIL : "MISSING"
      }`
    );
...
${finalHtmlBody}
    ...
    console.log(
      `[SendEmailFromAgent] Using A1Base Account ID: ${A1BASE_ACCOUNT_ID}`
    );
    ...
    try {
      ...
      if (!response.ok || responseData.status === "error") {
        ...
        console.error(
          `[SendEmailFromAgent] A1Base API returned error: ${errorMessage}`
        );
        throw new Error(`Failed to send email: ${errorMessage}`);
      }
    } catch (error: any) {
      console.error(
        `[SendEmailFromAgent] Error calling A1Base API directly:`,
        error
      );
      throw error;
    }
    ...
    console.log(
      `[SendEmailFromAgent] Email details - From: ${trimmedSenderEmail}, To: ${trimmedRecipientEmail}`
    );
    ...
    console.error(
      `[SendEmailFromAgent] Full error object:`,
...
export async function CreateEmailAddress(
  ...
  console.log(
    `[CreateEmailAddress] Attempting to create email address: ${fullEmail}`
  );

  if (!A1BASE_ACCOUNT_ID) {
    console.error(
      "[CreateEmailAddress] Missing A1Base Account ID in environment variables."
    );
    throw new Error(
      "Email address creation service is not properly configured."
    );
  }
  ...
  try {
    ...
    console.error(
      `[CreateEmailAddress] Error creating email address ${fullEmail}:`,
      error
    );
    throw error; // Re-throw to be handled by the caller
  }
