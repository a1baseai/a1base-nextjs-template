...
async function sendResponseMessage(
  ...
  for (const line of messageLines) {
    ...
    try {
      ...
      // Only log success if we actually succeeded
      console.log(
      ...
      // Re-throw the error so calling code knows the send failed
      throw new Error(`Failed to send ${threadType} message to ${recipientId}: ${error}`);
    }
...
export async function handleWhatsAppIncoming(
  ...
  if (thread_type === "group") {
    try {
      // TEMPORARY: Skip group onboarding check for debugging
      const SKIP_GROUP_ONBOARDING = true; // Set to false to re-enable
      
      if (!SKIP_GROUP_ONBOARDING && await isGroupInOnboardingState(thread_id)) {
        console.log(
  ...
  if (thread_type === "group" && adapter) {
    // TEMPORARY: Skip new group onboarding for debugging
    const SKIP_GROUP_ONBOARDING = true; // Set to false to re-enable
    ...
    if (!SKIP_GROUP_ONBOARDING && await handleGroupChatOnboarding(webhookData, isNewChatInDb)) {
  ...
  if (triageResponseMessageText) {
    ...
    try {
      ...
      console.log(`[DEBUG] sendResponseMessage completed successfully for ${recipient}`);
    } catch (sendError) {
      console.error(`[DEBUG] Failed to send response message:`, sendError);
      // Log the actual message that failed to send for debugging
      console.error(`[DEBUG] Failed message content (first 100 chars): ${triageResponseMessageText.substring(0, 100)}...`);
    }
